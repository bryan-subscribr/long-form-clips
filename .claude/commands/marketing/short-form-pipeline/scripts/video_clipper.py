...
Style: Default,Arial Black,120,&H00FFFFFF,&H0000FFFF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,8,2,2,60,60,320,1
Style: Highlight,Arial Black,120,&H0000D4FF,&H0000FFFF,&H00000000,&H80000000,-1,0,0,0,100,100,0,0,1,8,2,2,60,60,320,1
...
class VideoClipper:
    ...
    def create_ass_captions(self, words, clip_start, clip_end, clip_type, output_path):
        """Create ASS file with TikTok-style, CONTINUOUS word-highlighted captions.

        Shows 3 words at a time with the current word highlighted in yellow. Each
        chunk stays on screen continuously from when it starts until the next chunk
        begins (no blank gaps during pauses), so a caption is always visible.
        ...
        # Enforce strictly monotonic, non-overlapping word times. Coarse or
        # duplicated source timestamps (e.g. from caption tracks) otherwise make
        # multiple highlight events overlap and libass stacks them into a ladder.
        clip_words = sorted(clip_words, key=lambda w: w['start'])
        prev_end = clip_start
        normalized = []
        for w in clip_words:
            ws = max(w['start'], prev_end)
            we = max(w['end'], ws + 0.12)
            normalized.append({'word': w['word'], 'start': ws, 'end': we})
            prev_end = we
        clip_words = normalized

        is_vertical = clip_type == 'vertical_short'
        header = ASS_HEADER if is_vertical else ASS_HEADER_HORIZONTAL

        CHUNK_SIZE = 3
        chunks = [clip_words[i:i + CHUNK_SIZE]
                  for i in range(0, len(clip_words), CHUNK_SIZE)]

        clip_dur = clip_end - clip_start

        def rel(t):
            return min(clip_dur, max(0.0, t - clip_start))

        events = []
        for ci, chunk in enumerate(chunks):
            # Continuous display window: from this chunk's first word (or clip
            # start for the first chunk) until the NEXT chunk's first word (or
            # clip end for the last chunk). No gaps.
            disp_start = 0.0 if ci == 0 else rel(chunk[0]['start'])
            disp_end = clip_dur if ci == len(chunks) - 1 else rel(chunks[ci + 1][0]['start'])
            if disp_end <= disp_start:
                disp_end = min(clip_dur, disp_start + 0.3)

            n = len(chunk)
            for wi, w in enumerate(chunk):
                # Highlight tiles the display window so it's never blank: first
                # word owns the lead-in, last word owns the tail-out.
                hl_start = disp_start if wi == 0 else max(disp_start, rel(w['start']))
                hl_end = disp_end if wi == n - 1 else max(hl_start + 0.05, rel(chunk[wi + 1]['start']))
                hl_end = min(hl_end, disp_end)
                if hl_end <= hl_start:
                    continue
                ...
                start_ts = self._seconds_to_ass_time(hl_start)
                end_ts = self._seconds_to_ass_time(hl_end)
    ...
    def create_single_clip(self, video_path, segment, clip_type, output_path, captions=True):
        ...
        captions=False skips burned-in subtitles entirely (horizontal clips often
        ship clean and let the platform handle CC).
        """
